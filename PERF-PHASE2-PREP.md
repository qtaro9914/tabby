...
## 5.5 xterm.js消費天井の直接計測（2026-07-11、`scripts/perf/xterm-ceiling.mjs`）

PTY/IPC/ミドルウェアを完全にバイパスし、同じ50MBログをxterm.jsへ直接
`write()`して計測（devビルド、100KBチャンク、write完了コールバック待ち）:

| 経路 | スループット |
|---|---|
| フルパイプライン（参考: S1実測） | 10.2 MB/s |
| **可視ターミナルへ直接write**（パース+バッファ+描画） | **13.4 MB/s** |
| **detached Terminal**（パース+バッファのみ、レンダラなし） | **16.2 MB/s** |
| detached + Uint8Array入力（文字列デコード回避） | 16.6 MB/s |

**結論**:
1. 「残存上限はxterm.js本体」という推定を定量確認。パイプライン全体
   （PTY+IPC+ミドルウェア+Tabby書き込み経路）のオーバーヘッドは13.4→10.2の
   約24%に過ぎず、支配項はxterm.jsの端末エミュレーションコア（~16MB/s）
2. レンダラが「65% busyで飽和していない」のは余力ではなく、xterm.jsの
   WriteBufferが**約12msのタイムスライスでyieldする設計**（`WRITE_TIMEOUT_MS`、
   非設定項目）のため。飽和せずにペースメーカーになる
3. 文字列 vs Uint8Array のデコード差は~2% — xterm入力形式の変更単体では
   throughputはほぼ変わらない（GC圧削減の価値は別途あり）
4. フロー制御・チャンクサイズの追加チューニングは無意味と再確認
   （消費側が~13MB/sで律速のため）
